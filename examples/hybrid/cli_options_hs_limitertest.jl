# julia --color=yes --project=examples --threads=8 examples/hybrid/driver.jl --forcing held_suarez --moist equil --vert_diff true --microphy 0M --dt 400secs --apply_moisture_filter false --t_end 600days --fps 30 --job_id longrun_hs_rhoe --dt_save_to_sol 6hours --dt_save_to_disk 10days
import ArgParse
...
function cfl_acoustic_horizontal(Δt, Nq, ne)
    c = 383 # m/s
    radius = 6371e3 # m
    Δx = (2*pi*radius) / (ne * 4)  * minimum(diff(Spaces.Quadratures.quadrature_points(Float64, Spaces.Quadratures.GLL{Nq}())[1])) / 2
    cfl_global = c * Δt / Δx
end

using Plots
using ClimaCorePlots
function plot_state(Y, l = 1)
    p1 = plot(Y.c.ρe_tot, title = "e", level = l)
    p2 = plot(Y.c.ρq_tot, title = "q", level = l)
    p3 = plot(Geometry.UVVector.(Y.c.uₕ).components.data.:1, title = "u", level = l)
    p4 = plot(Geometry.UVVector.(Y.c.uₕ).components.data.:2, title = "v", level = l)
