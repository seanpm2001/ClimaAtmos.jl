...
const prior = CAL.get_prior(joinpath(experiment_dir, experiment_config["prior"]))
...
# CAL.initialize(ensemble_size, observations, prior, output_dir; scheduler = EKP.DefaultScheduler(0.25))
CAL.initialize(ensemble_size, observations, prior, output_dir; scheduler = EKP.DefaultScheduler(0.001))

eki = nothing
slurm_kwargs = CAL.kwargs(time = 60, mem = "16G")
