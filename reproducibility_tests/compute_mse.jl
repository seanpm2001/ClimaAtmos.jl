...
function reproducibility_test(;
    ...
    paths = nothing # initialize for later handling

    if haskey(ENV, "BUILDKITE_COMMIT")
        paths = self_reference_or_paths()
        if paths == :self_reference
            return (reference_mse, paths)
        else
            n_max_comparisons = 10
            n_paths = length(paths)
            # Get the top 10 most recent paths to compare against:
            paths = reverse(paths)[1:min(n_max_comparisons, n_paths)]
        end
